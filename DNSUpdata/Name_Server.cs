...
namespace DNSTest
{
    public partial class Name_Server : Form
    {
        ...
        public Name_Server()
        {
            ...
            collation = Database.GetCollection<BsonDocument>("NameServer");
            showdatabase();
        }
        ...
        void Receive()
        {
            ...
            while (true)
            {
                ...
                //Nếu không tìm thấy trả về Host Not Found
                else
                {
                    var sendBuffer = Encoding.UTF8.GetBytes("Host Not Found");
                    // gửi kết quả lại cho Resolve_Server
                    socket.SendTo(sendBuffer, remoteEndpoint);
                    Array.Clear(receiveBuffer, 0, size);
                }
            }
        ...
        private void nameserver_Load(object sender, EventArgs e)
        {
            CheckForIllegalCrossThreadCalls = false;
            Thread m = new Thread(new ThreadStart(Receive));
            m.Start();
        }
